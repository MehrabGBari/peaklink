function est=O18rateLinear_matrix_inv(p,intensity,min_f,max_f,rangerate,pp)
global p1;
global p2;
global p3;
global sigma1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
Su_inv_m=inv(P'*P)*P';
x=Su_inv_m*intensity';
Id=find(x<0);
if ~isempty(Id)
    x(Id)=0;
end
if x(1)==0 || x(2)==0 || x(3)==0
    f=0; theta2=0; theta1=0.00001;
else
    f=2*x(3)/(2*x(3)+x(2));
    theta2=x(3)/(f^2);
...
% %est=fmincon(@myLSEfunc, [init_a, init_rate, init_f],[],[],[],[],[rangerate*init_a lowerrate min_f],[1/rangerate*init_a upperrate 1],[],options);
...
%     options=optimset('Algorithm','active-set','Display','off');
%     try
%         est=fmincon(@myLSEfunc, [init_a, init_rate,init_f],[],[],[],[],[rangerate*init_a lowerrate min_f],[1/rangerate*init_a upperrate max_f],[],options);
%     catch exception

function [parameters,Result,SVMStruct]=BuildAWModelParameters_1(Training_Information_Matrix,Model_Training,parameters)
 ...
 %%%%%%%%%%%%%%%%%%
    ...
    for i=1:length(Model_Training)
        ...
        for j=1:length(Model_Training{i}.AW)
            if Model_Training{i}.AW(j)~=0
                PAW_corr_param (j)= pdf('Gamma',Model_Training{i}.AW(j),parameters.PAWa_AWmodel(2),parameters.PAWa_AWmodel(1));
                PAW_noncorr_param (j)= pdf('Gamma',Model_Training{i}.AW(j),parameters.PAWa_AWmodel(4),parameters.PAWa_AWmodel(3));
                PAWc_div_PAWnc(j)= PAW_corr_param (j)/ PAW_noncorr_param (j);
                PAWnc_div_PAWc(j)= PAW_noncorr_param (j)/ PAW_corr_param (j);
                PATc_div_PATnc(j)=Model_Training{i}.PAT_c(j)/Model_Training{i}.PAT_nc(j);
            else
                PAW_corr_param (j)= 0;
...
TrainData=[Corresponding_PAT(1:L2) NonCorresponding_PAT(1:L1);Corresponding_PAW(1:L2) NonCorresponding_PAW(1:L1)];
N1=length(TrainData);n_train=length(Corresponding_PAW);
T=[ones(n_train,1);zeros(N1-n_train,1)]; % the label vector
%T=T';
%figure;
SVMStruct = svmtrain(TrainData,T','KERNEL_FUNCTION','linear','BOXCONSTRAINT',30,'showplot',true);
GROUP =  svmclassify(SVMStruct,TrainData');
TrainAccuracy_SVM=mean(double(GROUP(1:L2,1)==T(1:L2,1))*100)
function [IntervalList,Jud_detect_good]=TOF_Verification_intervaldetection_Spec_Pepv1(Pep,iso,Orbi_ms2information,XICs,Orbi_retentiontl1,TOF_peakl,mzlist)
%%%%%%%%%%%%%%%%%%%%%%%%
%%%% this v1 function is for interval detection on 20ppm and then enlarge
%%%% the interval to 80ppm.
...
%             Interval_XIC=xic(Scan_start01:Scan_end01,j);
            MZ_interval(1)=mzlist(j)-mzlist(j)*80/10^6;
            MZ_interval(2)=mzlist(j)+mzlist(j)*80/10^6;              
            [Int_Matrix_mz_time,mz_unique,Pep_mz,Interval_XIC]=Generate_Pep_Mz_Time_matrix(TOF_peakl,MZ_interval,Scan_start01,Scan_end01);
            intervalid{j}.XICs_matrix_80ppm{k}=Int_Matrix_mz_time;
            intervalid{j}.Elution_profile_80ppm{k}=Interval_XIC;
...
%                     plot(Interval_XIC,'r');hold on;plot(Interval_XIC_afterfilter,'k')
        ...
        intervalid{j}.XICs_matrix_80ppm=0;
        intervalid{j}.Elution_profile_80ppm=0;
    end
end
...
if ~isempty(Total_interval)
    for k=1:size(Total_interval,1)
        ...
        for j=1:6
            ...
            MZ_interval(1)=mzlist(j)-mzlist(j)*80/10^6;
            MZ_interval(2)=mzlist(j)+mzlist(j)*80/10^6;    
            [Int_Matrix_mz_time,mz_unique,Pep_mz,intervaldata(:,j)]=Generate_Pep_Mz_Time_matrix(TOF_peakl,MZ_interval,Scan_start,Scan_end);
            IntervalList.intervalid_beforelong{k}.XICs_matrix_80ppm{j}=Int_Matrix_mz_time;
...
%             intervaldata(:,j)=XICs(Scan_start:Scan_end,j);
        ...
        IntervalList.intervalid_beforelong{k}.intervaldata=intervaldata;

function [IntervalList,Jud_detect_good]=SILAC_Verification_intervaldetection_Spec_Pepv1(iso,XICs, threshold)
...
%             plot(XIC_afterfilter_2ndtime,'y')
    %%%%%%%%%%%%
...
%                     plot(Interval_XIC_afterfilter_2ndtime,'m');plot(Interval_XIC_afterfilter_3rdtime);
            %%%%%
...
%                 Maxval_posi_after5{j}(k,:)=[max_value,max_posi-peak_start];
            ...
            if ~isempty(ID_interval_L)
                for rrr=1:length(ID_interval_L)
                 ...
                 intervallist_after5{j}(num,:)=[0 0];
                 Maxval_posi_after5{j}(num,:)=[0 0];
                 num=num+1;
            end
            %%%%%
...
if ~isempty(Total_interval)
    for k=1:size(Total_interval,1)
        ...
        intervaldata=zeros(Scan_end-Scan_start+1,8);
        intervalsdata01v1_afterfilter=zeros(Scan_end-Scan_start+1,8);
        N_mean_filter=5;
        for j=1:8
        ...
        [intervalsdata01v2 ,Scan_start_afterfilter,Scan_end_afterfilter]=MS2intervalfilter_SILAC(intervaldata, MS2_Peak_Intensity01, intervalsdata01v1_afterfilter, MS2_Peak_Intensity01_afterfilter, Scan_start, Scan_end, Ms2_scannumber, threshold);
...
%         IntervalList.Labelling_effeciency(k)=Labelling_efficiency;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%% check observed isopattern to the natural
        %%%%%%%%%%%%%%%%%%%%%%%%%% isotope pattern
        Ob_data=sum(intervalsdata01v2,1);
        Ob_data_L=Ob_data(1:4); Ob_data_L_pattern=Ob_data_L./sum(Ob_data_L);
        Ob_data_H=Ob_data(5:8); Ob_data_H_pattern=Ob_data_H./sum(Ob_data_H);
        IntervalList.Labelling_effeciency(k)=sum(Ob_data_H)/sum(Ob_data_L);
        log_KL_Value_L=KL_calculate(Ob_data_L,iso);
        log_KL_Value_H=KL_calculate(Ob_data_H,iso);
        if log_KL_Value_L<= threshold || log_KL_Value_H<=threshold%log_KL_Value_L<=-2.5 || log_KL_Value_H<=-2.5
            IntervalList.Judge(k)=1;
        else
            IntervalList.Judge(k)=0;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%% check interval length which should be longer than 2
Id_notzeros=find(IntervalList.intervallist_after7_combine_beforecheckL(:,1)~=0);
if ~isempty(Id_notzeros)
    Id_length5=find(IntervalList.intervallist_after7_combine_beforecheckL(Id_notzeros,2)-IntervalList.intervallist_after7_combine_beforecheckL(Id_notzeros,1)>=2);
...
%         est=O18rateLinear(iso(1:6),intensity,minf,maxf,rangerate,1);
        Final_Ob_data_L=intensity(1:4);
        Final_Ob_data_H=intensity(5:8);
        IntervalList.Labelling_efficiency_after7_combine(k)=sum(Final_Ob_data_H)/sum(Final_Ob_data_L);        
    else
